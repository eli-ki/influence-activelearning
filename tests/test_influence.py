...
def test_validate_influence_dataset_accepts_multiclass():
    y = np.array([0, 1, 2, 0, 1, 2])
    validate_influence_dataset(y, "classification", "cifar10")


def test_validate_influence_dataset_rejects_regression():
    y = np.array([0.1, 0.2, 0.3])
    with pytest.raises(ValueError, match="classification"):
        validate_influence_dataset(y, "regression", "california_housing")
...
def test_influence_scorer_multiclass():
    pytest.importorskip("tree_influence")
    X = np.random.randn(80, 4)
